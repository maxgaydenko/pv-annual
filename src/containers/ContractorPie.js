...
class ContractorPie extends React.Component {
 componentWillMount() {
  const yearData = window._pvad.contractorData.filter(f => f.year === window._pvad.currentYear)[0];
  const yearDataPrev = window._pvad.contractorData.filter(f => f.year === window._pvad.currentYear-1)[0];
  ...
  window.document.title = "Выручка по контрагентам за "+window._pvad.currentYear+" год";
 }
 ...
 render() {
  ...
  return (
   <div className="App-body">
    <header>
     <h1>Контрагенты за {window._pvad.currentYear} год</h1>
